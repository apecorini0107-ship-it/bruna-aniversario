...
const TOSS_DURATION = 2.5;
const TOSS_TOTAL_MS = TOSS_DURATION * 1000 + 300;
...
export default function SurpriseQuestion({ onYes }: { onYes: () => void }) {
  ...
  useEffect(() => {
    if (showJoke) {
      const t = setTimeout(() => setShowJoke(false), TOSS_TOTAL_MS + 400);
      return () => clearTimeout(t);
    }
  }, [showJoke]);
    ...
    <div className="flex min-h-[100dvh] w-full flex-col items-center justify-center px-6 py-16 text-center">
      ...
      >
        ...
        <div className="relative flex h-24 w-full items-center justify-center gap-5">
          ...
          {/* Nao button + its bin, stacked in the same spot: the bin is literally hidden behind the button until it falls in */}
          <div className="relative flex h-[64px] w-[96px] items-center justify-center">
            <motion.div
              aria-hidden
              className="pointer-events-none absolute text-bordeaux-dark/50"
              initial={{ opacity: 0, scale: 0.6 }}
              animate={
                tossing
                  ? { opacity: [0, 1, 1, 1, 0], scale: [0.6, 1, 1, 1.3, 0.7] }
                  : { opacity: 0, scale: 0.6 }
              }
              transition={{
                duration: TOSS_DURATION,
                times: tossing ? [0, 0.12, 0.6, 0.82, 1] : undefined,
                ease: "easeOut",
              }}
            >
              ...
              {!tossing && (
                <motion.button
                  ...
                  exit={{
                    y: [0, -24, -10, 22, 68],
                    x: [0, 10, -10, 4, 0],
                    rotate: [0, -12, 14, -8, 260],
                    scale: [1, 1.05, 1, 0.85, 0.1],
                    opacity: [1, 1, 1, 1, 0],
                    transition: {
                      duration: TOSS_DURATION,
                  ...
                  className="relative z-10 flex items-center justify-center rounded-full border border-bordeaux/25 bg-white/70 px-6 py-3 text-sm font-medium text-bordeaux-dark/80 backdrop-blur-sm"
                >
                  Não
                </motion.button>
              )}
